/**
 * @file    hub12.h
 * @brief   P10 单色点阵 LED 屏 HUB12 接口驱动
 *
 * HUB12 接口说明:
 ...
 *
 *   本驱动仅使用 R1 数据线驱动 32×16 单色 P10 模块。
 *   如需级联多块屏, 可扩展 R2 引脚驱动下半部分。
 *
 * 硬件引脚 (STM32F103C8T6):
 *   PB3  -> A    (行选地址位 0, LSB)
 *   PB4  -> B    (行选地址位 1, MSB)
 ...
 *
 * 显示缓冲区布局:
 *   64 字节, 每字节 8 bit, 逐行排列。
 *   byte[0~3]  = 第 0 行 (32 像素 / 8 = 4 字节)
 *   byte[4~7]  = 第 1 行
 *   ...
 *   byte[60~63] = 第 15 行
 */
...
/** @brief 上半屏数据线 R1 — 串行数据输入, 1=点亮该像素 */
 ...
 *   TIM2 参数: 预分频 72 (1MHz), 计数 1250 (800Hz 中断)
 *   4 组扫描 × 800Hz = 每组 200Hz → 总刷新率 ~50Hz, 无闪烁
 */
 ...
 *     1. 关闭输出 (OE=HIGH, 消隐)
 *     2. 设置地址线 A/B 选择当前组
 *     3. 从右到左逐列移入 4 bit 数据 (每列 4 个 CLK)
 *     4. 锁存 (STB 脉冲)
 *     5. 打开输出 (OE=LOW, 点亮)
 *     6. 组号 +1
 */
