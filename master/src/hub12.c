/**
 * @file    hub12.c
 * @brief   P10 单色点阵 LED 屏 HUB12 接口驱动实现 (级联双屏)
 *
 * ===================== HUB12 工作原理 =====================
 *
 ...
 *     1  1  │ 3,  7,  11, 15
 *
 *   每组扫描时, 先串行移入 64 列 × 4 bit = 256 个时钟 (级联双屏),
 *   然后锁存输出, 再切换到下一组。人眼视觉暂留效果使
 *   4 组快速交替点亮时看起来像是同时显示。
 *
 * 级联原理:
 *   两块 P10 面板通过级联输出连接。面板1的 DOUT 连接到面板2的 DIN。
 *   移入 64 列数据后, 面板1自动将后半部分数据级联到面板2。
 *
 * 数据移入顺序:
 *   从最右列 (col=63) 到最左列 (col=0) 逐列移入。
 *   前 32 列数据送给面板1, 后 32 列数据通过级联送给面板2。
...
/* ======================== 显示缓冲区 ======================== */
/**
 * 帧缓冲区 — 128 字节, 对应 64×16 = 1024 像素 (级联双屏)
 * 每 bit 代表一个像素, 1=亮, 0=灭
 * 排列方式: 逐行, 每行 8 字节 (64 像素 / 8 bit = 8 字节)
 *
 * v3.0 更新: 缓冲区从 64 字节扩展到 128 字节
 */
 ...
 * @retval 1=亮, 0=灭
 *
 * 缓冲区布局: byte_idx = y * 8 + (x / 8), bit_idx = x % 8
 *
 * v3.0 更新: x 范围从 0~31 扩展到 0~63 (级联双屏)
 */
...
void HUB12_Init(void)
{
     ...
     *   周期 = 2500-1 → 中断频率 = 1MHz / 2500 = 400 Hz
     *   4 组扫描 → 每组频率 = 400 / 4 = 100 Hz
     *   总刷新率 = 100 Hz (即每秒整个屏幕刷新 100 次, 无闪烁)
     *
     * v3.0 更新: 级联双屏需要更多时钟周期 (64列 vs 32列)
     */
    ...
    tim.TIM_Period = 2500 - 1;              /* 1MHz / 2500 = 400Hz (级联双屏) */
...
void HUB12_Refresh(void)
{
    ...
    /**
     * 从最右列 (col=63) 到最左列 (col=0) 逐列移入。
     * 前 32 列送给面板1, 后 32 列通过面板1级联输出送给面板2。
